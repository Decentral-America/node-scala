...
import com.decentralchain.ride.runner.requests.*
...
class RequestServiceTestSuite extends BaseTestSuite with HasGrpc with HasBasicGrpcConverters with HasTestDb {
  ...
  protected def test(requestServiceSettings: DefaultRequestService.Settings = defaultRequestServiceSettings)(f: TestDependencies => Unit): Unit =
    Using.Manager { use =>
      ...
      val requestsService =
        use(new DefaultRequestService(requestServiceSettings, blockchain, allTags, use(new TestJobScheduler[RideScriptRunRequest]()), testScheduler))
