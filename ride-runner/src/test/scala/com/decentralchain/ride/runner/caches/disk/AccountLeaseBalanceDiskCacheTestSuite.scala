...
import com.decentralchain.ride.runner.db.ReadWrite
import com.decentralchain.state.{Height, LeaseBalance}
...
class AccountLeaseBalanceDiskCacheTestSuite extends DiskTestSuite {
  ...
  private def test(f: DiskCache[Address, LeaseBalance] => ReadWrite => Unit): Unit = withDb { db =>
    db.directReadWrite { implicit ctx =>
      f(DefaultDiskCaches(db).accountLeaseBalances)(ctx)
    }
  }
}
