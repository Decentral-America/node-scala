...
import com.decentralchain.state.{AccountScriptInfo, AssetDescription, DataEntry, Height, IntegerDataEntry, LeaseBalance, TransactionId}
import com.decentralchain.transaction.{Asset, AssetIdLength}
import com.decentralchain.utils.StringBytes
import com.decentralchain.{BaseTestSuite, HasTestAccounts, state}
...
class LazyBlockchainTestSuite extends BaseTestSuite with HasTestDb with HasGrpc with HasTestAccounts {
        ...
        "block" - {
          ...
          "updates activated features" in withDb { db =>
            db.directReadWrite { implicit rw =>
              ...
              blockchain.activatedFeatures shouldBe DefaultBlockchainSettings.functionalitySettings.preActivatedFeatures.view
                .mapValues(Height.apply)
                .toMap
  ...
  private val pbStateUpdate = StateUpdate.defaultInstance
    ...
    .withAssets(
      Seq(
        com.decentralchain.events.StateUpdate.AssetStateUpdate.toPB(
          com.decentralchain.events.StateUpdate.AssetStateUpdate(
            asset.id,
            before = none,
