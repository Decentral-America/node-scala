...
import com.decentralchain.api.observers.{ManualGrpcObserver, MonixWrappedDownstream}
  ...
  override def getAssetDescription(asset: Asset.IssuedAsset): Option[AssetDescription] = {
    ...
    val r = xs.map { x =>
      AssetDescription(
        ...
        // All next fields are not used, see: https://docs.decentralchain.io/en/ride/structures/common-structures/asset#fields
...
object DefaultBlockchainApi {
  ...
  def toVanilla(blockWithHeight: BlockWithHeight): Option[SignedBlockHeaderWithVrf] = for {
    ...
    signedBlock <- PBBlocks.vanilla(b).toOption
