...
import com.decentralchain.crypto.Curve25519.KeyLength
import com.decentralchain.generator.utils.Implicits.*
  ...
  def transfers(senderGen: Iterator[KeyPair], recipientGen: Iterator[Address], feeGen: Iterator[Long]): Iterator[Transaction] = {
    ...
    senderGen
      ...
      .map { case (((src, dst), fee), i) =>
        TransferTransaction
          .create(2.toByte, src.publicKey, dst, Dcc, fee, Dcc, fee, ByteStr.empty, now + i, Proofs.empty)
    ...
    senderGen
      ...
      .map { case ((sender, count), i) =>
        ...
        MassTransferTransaction
          .create(1.toByte, sender.publicKey, Dcc, transfers, fee, now + i, ByteStr.empty, Proofs.empty)
