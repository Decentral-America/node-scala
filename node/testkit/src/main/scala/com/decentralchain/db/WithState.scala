...
import com.decentralchain.block.Block.{GenesisBlockVersion, GenesisGenerationSignature, GenesisGenerator, GenesisReference}
import com.decentralchain.common.state.ByteStr
import com.decentralchain.common.utils.EitherExt2.*
import com.decentralchain.database.{KeyTag, RDB, RocksDBWriter, TestStorageFactory, loadActiveLeases}
...
import com.decentralchain.mining.{Miner, MiningConstraint}
import com.decentralchain.settings.{TestFunctionalitySettings as TFS, *}
...
trait WithState extends BeforeAndAfterAll with DBCacheSettings with Matchers with NTPTime { suite: Suite =>
  ...
  protected def withRocksDBWriter[A](ws: DCCSettings)(test: RocksDBWriter => A): A = {
    try {
  ...
  protected def withTestState[A](ws: DCCSettings)(test: (BlockchainUpdaterImpl, RocksDBWriter) => A): A = {
    try {
...
trait WithDomain extends WithState { suite: Suite =>
  val DomainPresets = com.decentralchain.test.DomainPresets
