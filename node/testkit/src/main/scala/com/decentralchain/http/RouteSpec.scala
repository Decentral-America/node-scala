...
import com.decentralchain.api.http.ApiMarshallers
...
abstract class RouteSpec(basePath: String) extends FreeSpec with ScalatestRouteTest with ApiErrorMatchers with JsonMatchers with ApiMarshallers {
  ...
  implicit class RouteTestingOps(route: Route) {
    ...
    /** Convenient utility for testing multi-routes created with the [[com.decentralchain.api.http.CustomDirectives#anyParam(java.lang.String)]]
      * directive
      * @param baseUrl
      *   Base route URL
      * @param paramName
