...
class HotStuffViewChangeSpecification extends FlatSpec {
  ...
  private def committeeVoteFor(view: Int, phase: HotStuffPhase, blockId: BlockId, height: Int, voterIndex: Int): HotStuffVote = {
    ...
    HotStuffVote(view, phase, blockId, com.decentralchain.state.Height(height), voterIndex, kps(voterIndex).sign(msg, BlsUtils.BlsHsVoteDomainSeparationTag).byteStr)
  }
  ...
  "onRoundTimerTick's leader-timeout re-propose choice" should
    "re-propose the in-flight prepareQC'd-but-not-committed branch instead of jumping to blockSource's settled tip" in {
      ...
      Seq(0, 1).foreach { i =>
        ...
        node2.onVote(HotStuffVote(0, HotStuffPhase.HOTSTUFF_PHASE_PREPARE, B, com.decentralchain.state.Height(H), i, kps(i).sign(msg, BlsUtils.BlsHsVoteDomainSeparationTag).byteStr))
      }
