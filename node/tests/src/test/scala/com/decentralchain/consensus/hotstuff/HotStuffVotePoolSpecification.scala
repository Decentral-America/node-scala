...
class HotStuffVotePoolSpecification extends FlatSpec {
  ...
  private def voteAtHeight(i: Int, h: Int): HotStuffVote = {
    ...
    HotStuffVote(5, PREPARE, block, Height(h), i, kps(i).sign(msg, BlsUtils.BlsHsVoteDomainSeparationTag).byteStr)
  }
  ...
  it should "pool distinct targets separately" in {
    ...
    val voteOther      = {
      ...
      HotStuffVote(5, PREPARE, other, Height(height), 1, kps(1).sign(msg, BlsUtils.BlsHsVoteDomainSeparationTag).byteStr)
    }
    val (p1, _) = HotStuffVotePool.onVote(VotePool(), vote(0), committee)
