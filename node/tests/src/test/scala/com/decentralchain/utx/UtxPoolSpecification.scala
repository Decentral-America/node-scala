...
class UtxPoolSpecification extends FreeSpec, BlocksTransactionsHelpers, WithDomain, EitherValues, Eventually {
  ...
  private def withMassTransferWithBlacklisted[A](allowRecipients: Boolean)(test: (KeyPair, UtxPoolImpl, Seq[MassTransferTransaction]) => A): A = {
    withState { case (sender, senderBalance, bcu) =>
      ...
      val time       = TestTime()
      // NOTE: Consider randomizing transaction data for better coverage
  ...
  "UTX Pool" - {
    ...
    "correctly process constraints in packUnconfirmed" in {
      withDomain(com.decentralchain.history.TransfersV2ActivatedAt0DCCSettings) { d =>
    ...
    "smart accounts" - {
      ...
      "dApp to dApp call chain in force validate mode" in withDomain(RideV5) { d =>
        ...
        val utx = new UtxPoolImpl(
          ...
          DefaultDCCSettings.maxTxErrorLogSize,
          isMiningEnabled = true
        )
      ...
      "InvokeScriptTransaction is fully validated in forceValidate mode, on alwaysUnlimitedExecution = true and before 1000 complexity otherwise" in withDomain(
        ...
        val unlimitedUtx = utx.copy(utxSettings = DefaultDCCSettings.utxSettings.copy(alwaysUnlimitedExecution = true))
      ...
      "correct events for InvokeScriptTransaction with big complexity on alwaysUnlimitedExecution = true" in withDomain(
        ...
        event.snapshot.balances((secondAddress, Dcc)) shouldBe d.blockchainUpdater.balance(secondAddress, Dcc) - 1
        event.snapshot.balances((recipient, Dcc)) shouldBe d.blockchainUpdater.balance(recipient, Dcc) + 1
        utx.close()
      }
      ...
      "invoke expression" in {
        ...
        withDomain(settings.copy(featuresSettings = settings.featuresSettings.copy(autoShutdownOnUnsupportedFeature = false))) { d =>
          ...
          val utx    = new UtxPoolImpl(ntpTime, d.blockchainUpdater, DefaultDCCSettings.utxSettings, DefaultDCCSettings.maxTxErrorLogSize, true)
    ...
    "cleanup" - {
      "doesnt take the composite snapshot into account" in withDomain() { d =>
        d.helpers.creditDccToDefaultSigner(11.dcc)
        val transfers = Seq.fill(10)(TxHelpers.transfer(amount = 10.dcc))
      ...
      "doesnt validate transactions which are removed" in {
        ...
        forAll(gen) { case (acc, _, tx1, rest) =>
          ...
          utx = new UtxPoolImpl(
            ntpTime,
            blockchain,
            DCCSettings.default().utxSettings,
            DCCSettings.default().maxTxErrorLogSize,
            isMiningEnabled = true
          )
