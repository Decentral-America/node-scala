...
import com.decentralchain.db.{DBCacheSettings, WithState}
...
class InvokeScriptTransactionCrosscontractInvokeDiffTest extends PropSpec with WithState with DBCacheSettings with EitherValues {
  ...
  property("Crosscontract call - internal invoke state update") {
    ...
    val scenario =
      for {
        ...
        invokeTx = TxHelpers.invoke(
          dApp = mainAcc.toAddress,
          func = Some("foo"),
          payments = payments,
          invoker = invoker,
          fee = fee,
    ...
    val scenario =
      for {
        ...
        ssTxMain = SetScriptTransaction
          .create(1.toByte, mainAcc.publicKey, Some(contractMain), fee, ts + 5, Proofs.empty)
  ...
  property("Crosscontract call - multiple internal invokes - state update") {
    ...
    def contractMain(secondAcc: Address, thirdAcc: Address, paymentAsset: ByteStr): Script =
    ...
    def contractSecond(thirdAcc: Address, transferAsset: ByteStr): Script =
      TestCompiler(V5).compileContract(s"""
                                          |{-# STDLIB_VERSION 5 #-}
                                          |{-# CONTENT_TYPE DAPP #-}
                                          |{-#SCRIPT_TYPE ACCOUNT#-}
    ...
    val scenario =
      for {
        ...
        ssTxSecond = SetScriptTransaction
          ...
          .map(_.signWith(secondAcc.privateKey))
          .explicitGet()
        ssTxThird = SetScriptTransaction
          .create(1.toByte, thirdAcc.publicKey, scriptThird, fee, ts + 5, Proofs.empty)
          .map(_.signWith(thirdAcc.privateKey))
          .explicitGet()
