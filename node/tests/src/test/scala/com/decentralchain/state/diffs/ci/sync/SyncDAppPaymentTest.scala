  ...
  property("negative sync dApp payments amount rejects tx after enforceTransferValidationAfter") {
    for {
      ...
      withDomain(RideV5.configure(_.copy(enforceTransferValidationAfter = 4)), balances) { d =>
        ...
        d.liquidSnapshot.balances((dApp1, asset)) shouldBe d.rocksDBWriter.balance(dApp1, asset) + 1
        d.liquidSnapshot.balances((dApp2, asset)) shouldBe d.rocksDBWriter.balance(dApp2, asset) - 1
        ...
        d.appendBlockE(invoke2) should produce {
          if (customAsset)
            ...
            s"DApp $dApp1 invoked DApp $dApp2 with attached DCC amount = -1"
        }
      }
    }
  }
