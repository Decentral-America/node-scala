...
import com.decentralchain.lang.v1.ContractLimits.*
import com.decentralchain.settings.{FunctionalitySettings, TestFunctionalitySettings, DCCSettings}
...
class InvokeScriptActionLimitsTest extends PropSpec with WithDomain with DBCacheSettings with EitherValues {
  ...
  Seq(V5, V6).foreach { version =>
    ...
    property(s"Allow not more $limit ScriptTransfer/Lease/LeaseCancel actions for V${version.id}") {
      ...
      assertDiffAndState(Seq(TestBlock.create(preparingTxs1)), TestBlock.create(Seq(invoke1), Block.ProtoBlockVersion), features(version)) {
        case (snapshot, bc) =>
          snapshot.scriptResults(invoke1.id()).error shouldBe None
  ...
  private def checkBalanceActionsLimits(rootVersion: StdLibVersion, nestedVersion: StdLibVersion, settings: DCCSettings)(
  ...
  private def checkAssetActionsLimits(rootVersion: StdLibVersion, nestedVersion: StdLibVersion, settings: DCCSettings)(
      equalV5LessV6Result: Matcher[Either[?, ?]],
      moreV5EqualV6Result: Matcher[Either[?, ?]],
      moreV5MoreV6Result: Matcher[Either[?, ?]],
      moreV5LessV6Result: Matcher[Either[?, ?]]
