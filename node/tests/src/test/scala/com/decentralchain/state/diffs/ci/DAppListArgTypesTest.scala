...
import com.decentralchain.features.BlockchainFeatures.{BlockV5, Ride4DApps, SynchronousCalls, RideV6 as RideV6F}
...
class DAppListArgTypesTest extends PropSpec with WithDomain with Inside {
  ...
  private def preconditions(dAppScript: Script, args: List[EVALUATED]) = {
    ...
    val setDApp = TxHelpers.setScript(dApp, dAppScript, 0.01.dcc, 1.toByte)
    val ci      = () => TxHelpers.invoke(dApp.toAddress, Some("f"), args, Nil, invoker, fee, Dcc, 1.toByte, ts)
