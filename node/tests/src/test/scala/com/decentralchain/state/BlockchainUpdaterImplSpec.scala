...
import com.decentralchain.history.{chainBaseAndMicro, randomSig}
...
import com.decentralchain.utils.{Schedulers, SystemTime, Time}
import com.decentralchain.{EitherMatchers, NTPTime}
...
class BlockchainUpdaterImplSpec extends FreeSpec with EitherMatchers with WithDomain with NTPTime with DBCacheSettings with MockFactory {
  ...
  "BlockchainUpdater should replace current liquid block with better one" in {
    ...
    withDomain(ConsensusImprovements, AddrWithBalance.enoughBalances(currentBlockSender, anotherBlockSender)) { d =>
      ...
      val appender =
        BlockAppender(d.blockchainUpdater, SystemTime, d.utxPool, d.posSelector, BlockEndorser.Disabled, scheduler, verify = false, txSignParCheck = false)(_, None)
