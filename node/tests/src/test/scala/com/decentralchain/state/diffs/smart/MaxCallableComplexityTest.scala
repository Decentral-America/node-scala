...
import com.decentralchain.account.{Address, AddressScheme, KeyPair}
...
import com.decentralchain.transaction.{GenesisTransaction, Proofs, TxVersion, TransactionSignOps}
...
class MaxCallableComplexityTest extends PropSpec with WithDomain with TransactionGenBase {
  ...
  property("max callable complexity for dApp script callable (V5) is 10000") {
    ...
    withDomain(DomainPresets.RideV5) { d =>
      ...
      val setLargeScript = SetScriptTransaction.selfSigned(TxVersion.V2, dApp, Some(largeScript(V5, 50)), 0.01.dcc, ts).explicitGet()
  ...
  property("max callable complexity for dApp script callable (V6) is 52000") {
    ...
    withDomain(DomainPresets.RideV6) { d =>
      ...
      val setScript      = SetScriptTransaction.selfSigned(TxVersion.V2, dApp, Some(largeScript(V6, 285)), 0.021.dcc, ts).explicitGet()
      val setLargeScript = SetScriptTransaction.selfSigned(TxVersion.V2, dApp, Some(largeScript(V6, 300)), 0.022.dcc, ts).explicitGet()
