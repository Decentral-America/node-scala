...
import com.decentralchain.it.util.AddressOrAliasExt
...
import com.decentralchain.lang.v1.traits.domain.{Lease, Recipient}
import com.decentralchain.settings.{FunctionalitySettings, TestFunctionalitySettings}
import com.decentralchain.state.{LeaseBalance, unsafeSummarizer}
...
import com.decentralchain.transaction.TxHelpers.{defaultAddress, invoke, lease, secondAddress, secondSigner, setScript}
...
class LeaseActionDiffTest extends PropSpec with WithDomain {
  ...
  private def leasePreconditions(
    ...
    val leaseTxAmount1 = 5.dcc
    val leaseTxAmount2 = 10.dcc
    ...
    val leaseAmount = customAmount.getOrElse(2.dcc)
  ...
  property(s"Lease action by address (invoker - recipient)") {
    ...
    withDomain(domainSettingsWithFS(v5Features)) { d =>
      ...
      val invokerPortfolio = d.blockchain.dccPortfolio(invoker)
      ...
      val dAppPortfolio = d.blockchain.dccPortfolio(dAppAcc)
  ...
  property(s"Lease action cancelled by LeaseCancelTransaction") {
    ...
    withDomain(RideV5) { d =>
      ...
      d.liquidSnapshot.balances((invoker, Dcc)) shouldBe d.rocksDBWriter.balance(invoker) - invoke.fee.value
      d.liquidSnapshot.balances((dAppAcc, Dcc)) shouldBe d.rocksDBWriter.balance(dAppAcc) - leaseCancelTx.fee.value
    }
  }
  ...
  property(s"Lease action with unexisting alias") {
    ...
    assertDiffAndState(
      ...
      snapshot.errorMessage(invoke.id()).get.text shouldBe "Alias 'alias:?:alias2' does not exist."
    }
  }
  ...
  property(s"Lease action with empty amount") {
    ...
    assertDiffAndState(
      ...
      snapshot.errorMessage(invoke.id()).get.text shouldBe "NonPositiveAmount(0,dcc)"
    }
  }
  ...
  property(s"Lease action spends all dApp balance") {
    ...
    assertDiffAndState(
      ...
      blockchain.dccPortfolio(dAppAcc).effectiveBalance(false).explicitGet() shouldBe 0
    }
  }
  ...
  Seq(V5, V6).foreach { version =>
    ...
    property(s"$limit Lease actions for V${version.id}") {
      ...
      assertDiffAndState(
        ...
        snapshot.balances((invoker, Dcc)) shouldBe ENOUGH_AMT - invoke.fee.value
  ...
  property(s"LeaseCancel action with Lease action from same result") {
    ...
    withDomain(domainSettingsWithFS(v5Features)) { d =>
      ...
      val recipientPortfolio = d.blockchain.dccPortfolio(recipient)
  ...
  property("ScriptTransfer after LeaseCancel of transferring balance") {
    withDomain(
      RideV5.configure(_.copy(blockVersion3AfterHeight = 0)),
      Seq(AddrWithBalance(secondAddress, 1.006.dcc))
    ) { d =>
