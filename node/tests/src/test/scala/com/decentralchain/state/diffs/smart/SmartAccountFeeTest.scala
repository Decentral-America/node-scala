...
import com.decentralchain.state.EmptyDataEntry
import com.decentralchain.state.diffs.FeeValidation.{FeeConstants, FeeUnit}
import com.decentralchain.test.{PropSpec, produce}
import com.decentralchain.transaction.{Transaction, TransactionType, TxHelpers, TxWithFee}
  ...
  private def appendAndAssertNotEnoughFee(tx: Transaction & TxWithFee, d: Domain) = {
    d.appendBlockE(tx) should produce(
      "TransactionValidationError(cause = GenericError(Transaction sent from smart account. " +
        ...
        s"Fee for ${tx.tpe.transactionName} (${tx.fee} in DCC) " +
        s"does not exceed minimal value of ${FeeConstants(tx.tpe) * FeeUnit + ScriptExtraFee} DCC.)"
    )
  }
