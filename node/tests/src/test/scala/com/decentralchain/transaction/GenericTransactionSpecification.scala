...
abstract class GenericTransactionSpecification[T <: Transaction] extends PropSpec {
  ...
  def transactionParser: com.decentralchain.transaction.TransactionParser
  def updateProofs(tx: T, p: Proofs): T
  def generator: Gen[(Seq[com.decentralchain.transaction.Transaction], T)]
  ...
  property(s"$transactionName id doesn't depend on proof") {
    forAll(generator, proofsGen) { case ((_, tx), proofs1) =>
