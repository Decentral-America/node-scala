package com.decentralchain.transaction.data
...
import com.decentralchain.state.{BinaryDataEntry, BooleanDataEntry, DataEntry, IntegerDataEntry, StringDataEntry}
...
class DataTransactionNewLimitsSpec extends FlatSpec with WithDomain {
  ...
  "Data transaction" should "handle new limits" in withDomain(DomainPresets.RideV6) { d =>
    ...
    val dataTransaction         = TxHelpers.dataV2(TxHelpers.defaultSigner, dataEntries, fee = 0.162.dcc)
  ...
  it should "have lowered fee after V6" in {
    def calculateFee(preset: DCCSettings)(data: DataEntry[?]*): Long =
      withDomain(preset)(d => d.commonApi.calculateDccFee(TxHelpers.dataWithMultipleEntries(TxHelpers.defaultSigner, data)))

    val bigData = (1 to 100).map(i => BinaryDataEntry(i.toString, ByteStr(new Array[Byte](1024 - i.toString.length))))
    calculateFee(DomainPresets.RideV5)(bigData*) shouldBe 0.101.dcc
    calculateFee(DomainPresets.RideV6)(bigData*) shouldBe 0.100.dcc
  }
