...
class BlockchainUpdaterTest extends FreeSpec with HistoryTest with WithDomain with HasExitInterceptor {
  ...
  private val DCCSettings = history.DefaultDCCSettings.copy(
    blockchainSettings = history.DefaultDCCSettings.blockchainSettings.copy(
      functionalitySettings = history.DefaultDCCSettings.blockchainSettings.functionalitySettings
    ...
    featuresSettings = history.DefaultDCCSettings.featuresSettings.copy(autoShutdownOnUnsupportedFeature = true)
  )

  private val DCCSettingsWithDoubling = DCCSettings.copy(
    blockchainSettings = DCCSettings.blockchainSettings.copy(
      functionalitySettings =
        DCCSettings.blockchainSettings.functionalitySettings.copy(preActivatedFeatures = Map.empty, doubleFeaturesPeriodsAfterHeight = 300)
    )
  )
  ...
  "features approved and accepted as height grows" in withDomain(DCCSettings) { domain =>
  ...
  "features rollback with block rollback" in withDomain(DCCSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "multiple features activation: one after another" - {
    ...
    "without votes for already activated feature" in withDomain(DCCSettings) { domain =>
      val b = domain.blockchainUpdater
      b.processBlock(genesisBlock)
    ...
    "with votes for already activated feature" in withDomain(DCCSettings) { domain =>
      val b = domain.blockchainUpdater
      b.processBlock(genesisBlock)
    ...
    "with multiple voting periods rollback" in withDomain(DCCSettings) { domain =>
      val b = domain.blockchainUpdater
      b.processBlock(genesisBlock)
  ...
  "features activation after rollback and appending blocks" - {
    ...
    "when not enough votes after rollback - should not approve and activate the feature" in withDomain(DCCSettings) { domain =>
      val b = domain.blockchainUpdater
    ...
    "when enough votes after rollback - should approve and activate the feature" in withDomain(DCCSettings) { domain =>
      val b = domain.blockchainUpdater
  ...
  "feature activation height is not overridden with further periods" in withDomain(DCCSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "feature activated only by 90% of blocks" - {
    "negative with insufficient votes" in withDomain(DCCSettings) { domain =>
      val b = domain.blockchainUpdater
      b.processBlock(genesisBlock)
    ...
    "positive" in withDomain(DCCSettings) { domain =>
      val b = domain.blockchainUpdater
      b.processBlock(genesisBlock)
  ...
  "features votes resets when voting window changes" in withDomain(DCCSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "block processing should fail if unimplemented feature was activated on blockchain when autoShutdownOnUnsupportedFeature = yes and exit with code 38" in withDomain(
    ...
    withExitInterceptor(UnsupportedFeature) { signal =>
      ...
      Try(b.processBlock(getNextTestBlockWithVotes(b, Seq(-1)))).recover[Any] {
  ...
  "sunny day test when known feature activated" in withDomain(DCCSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "empty blocks should not disable activation" in withDomain(DCCSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods works in the middle of activation period" in withDomain(DCCSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods should work after defined height" in withDomain(DCCSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
