...
import com.decentralchain.block.serialization.{BlockHeaderSerializer, BlockSerializer}
...
class BlockV5Test extends FlatSpec with WithMiner with OptionValues with EitherValues with BlocksTransactionsHelpers {
  ...
  "blockId" should "be header hash" in {
    ...
    forAll(preconditions) { case (_, genesis) =>
      ...
      withDomain(DCCSettings.default().copy(blockchainSettings = DCCSettings.default().blockchainSettings.copy(functionalitySettings = fs))) { d =>
