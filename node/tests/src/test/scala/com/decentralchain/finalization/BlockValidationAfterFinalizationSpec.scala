...
class BlockValidationAfterFinalizationSpec extends BaseFinalizationSpec {
  ...
  "should not append an invalid block" - {
          ...
          d.createBlock(
            txs = Seq(
              TxHelpers.transfer(
                ...
                amount = d.blockchain.balance(committedGenerator2Addr) - CommitToGenerationTransaction.DepositInDcclets - 2.dcc
              )
            ),
    ...
    "more than max valid endorsers" in new BaseTest {
      ...
      override def settings: DCCSettings = super.settings.configure(_.copy(maxValidEndorsers = 1))
  ...
  private trait BaseTest {
    ...
    def settings: DCCSettings = defaultSettings
