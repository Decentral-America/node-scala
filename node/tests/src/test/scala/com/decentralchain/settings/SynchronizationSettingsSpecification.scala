...
import com.decentralchain.network.InvalidBlockStorageImpl.InvalidBlockStorageSettings
import com.decentralchain.settings.SynchronizationSettings.{HistoryReplierSettings, MicroblockSynchronizerSettings, UtxSynchronizerSettings}
  ...
  "SynchronizationSettings" should "read values" in {
    val config = ConfigFactory
      .parseString("""
                     ...
                     |    max-base-target = 130
                     |
                     |    blacklist-on-score-mismatch = true
                     |
    ...
    val settings = ConfigSource.fromConfig(config).at("dcc.synchronization").loadOrThrow[SynchronizationSettings]
    ...
    settings.blacklistOnScoreMismatch shouldBe true
