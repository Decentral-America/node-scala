...
import com.decentralchain.features.BlockchainFeatures.{BlockReward, BlockRewardDistribution, ConsensusImprovements}
...
class BlockRewardSpec extends FreeSpec with WithDomain {
  ...
  private def boostBlockRewardActivationScenario(
    ...
    (1 to 3).foreach(_ => d.appendBlock(d.createBlock(Block.RewardBlockVersion, Seq.empty, generator = blockMiner, rewardVote = 6.dcc + rewardDelta)))
