...
import com.decentralchain.account.{AddressOrAlias, Alias, KeyPair}
...
import com.decentralchain.settings.{Constants, FunctionalitySettings}
import com.decentralchain.state.{Blockchain, LeaseBalance}
  ...
  "Upon feature activation" - {
    "expired leases are cancelled" in forAll(simpleScenario) { case (lessor, alias, genesis, b, emptyBlocks) =>
      withDomain(leasingSettings) { d =>
        ...
        leasesToBeCancelled.foreach { lt =>
          d.blockchainUpdater.leaseDetails(lt.id()).map(_.isActive) shouldBe Some(true)
        }
        ...
        leasesToBeCancelled.foreach { lt =>
          d.blockchainUpdater.leaseDetails(lt.id()).map(_.isActive) shouldBe Some(false)
        }
      }
    }
  }
