...
class BlockchainUpdaterMicroblockSunnyDayTest extends PropSpec with DomainScenarioDrivenPropertyCheck {
  ...
  val preconditionsAndPayments: Gen[Setup] = for {
    ...
    masterToAlice: TransferTransaction <- dccTransferGeneratorP(ts, master, alice.toAddress)
    aliceToBob  = createDccTransfer(alice, bob.toAddress, masterToAlice.amount.value - fee - 1, fee, ts).explicitGet()
    aliceToBob2 = createDccTransfer(alice, bob.toAddress, masterToAlice.amount.value - fee - 1, fee, ts + 1).explicitGet()
  ...
  property("all txs in one block: B0 <- B0m1 <- B0m2 <- B0m3!") {
    scenario(preconditionsAndPayments, MicroblocksActivatedAt0DCCSettings) { case (domain, (genesis, masterToAlice, aliceToBob, aliceToBob2)) =>
      ...
      domain.blockchainUpdater.processMicroBlock(microBlocks(2), None) should produce("unavailable funds")
  ...
  property("discards some of microBlocks: B0 <- B0m1 <- B0m2; B0m1 <- B1") {
    scenario(preconditionsAndPayments, MicroblocksActivatedAt0DCCSettings) { case (domain, (genesis, masterToAlice, aliceToBob, aliceToBob2)) =>
      ...
      domain.blockchainUpdater.processMicroBlock(microBlocks0(1), None) should beRight
  ...
  property("discarding some of microBlocks doesn't affect resulting state") {
    forAll(preconditionsAndPayments, accountGen) { case ((genesis, masterToAlice, aliceToBob, aliceToBob2), miner) =>
      ...
      val minerABalance = withDomain(MicroblocksActivatedAt0DCCSettings) { da =>
      ...
      val minerBBalance = withDomain(MicroblocksActivatedAt0DCCSettings) { db =>
