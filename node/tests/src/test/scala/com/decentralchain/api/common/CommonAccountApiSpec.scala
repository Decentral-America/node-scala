...
import com.decentralchain.state.{DataEntry, EmptyDataEntry, Height, StringDataEntry, TransactionId, diffs}
import com.decentralchain.test.DomainPresets.RideV4
import com.decentralchain.test.*
import com.decentralchain.transaction.TxHelpers.data
import com.decentralchain.transaction.TxVersion.V2
import com.decentralchain.transaction.{GenesisTransaction, TxHelpers}
import com.decentralchain.{BlocksTransactionsHelpers, history}
  ...
  "Data stream" - {
    ...
    "filters liquid entities" in {
      ...
      val preconditions = for {
        ...
        data1          = TxHelpers.data(acc, Seq(entry1), fee)
        data2          = TxHelpers.data(acc, Seq(entry2), fee)
        data3          = TxHelpers.data(acc, Seq(entry3), fee)
