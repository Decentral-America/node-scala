package com.decentralchain.api.eth
...
import com.decentralchain.test.node.{randomAddress, randomKeyPair}
...
class EthRpcRouteSpec extends RouteSpec("/eth") with WithDomain with EthHelpers {
  ...
  "eth_chainId" in withDomain(DefaultDCCSettings) { d =>
    routeTest(d, "eth_chainId")(resultInt shouldBe DefaultDCCSettings.blockchainSettings.addressSchemeCharacter.toLong)
  }
  ...
  "absence of id" in withDomain() { d =>
    Post(routePath(""), Json.obj("method" -> "eth_chainId"))
      .~>(new EthRpcRoute(d.blockchain, d.commonApi.transactions, ntpTime).route)
      .~>(check { responseAs[JsObject] shouldBe Json.obj("id" -> null, "jsonrpc" -> "2.0", "result" -> "0x21") })
  }
