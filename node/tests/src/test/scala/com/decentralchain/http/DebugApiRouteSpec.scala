...
import com.decentralchain.api.http.DebugApiRoute.AccountMiningInfo
...
import com.decentralchain.lang.directives.values.{V4, V5, V6}
...
import com.decentralchain.state.{Blockchain, Height, StateHash}
...
class DebugApiRouteSpec
  ...
  override def settings: DCCSettings = DomainPresets.ContinuationTransaction.copy(
  ...
  routePath("/balances/history/{address}") - {
    ...
    "works" in {
      ...
      Get(routePath(s"/balances/history/${acc2.toAddress}")) ~> ApiKeyHeader ~> route ~> check {
  ...
  routePath("/stateHash") - {
    "works" - {
      "at nonexistent height" in {
        Get(routePath(s"/stateHash/${domain.blockchain.height}")) ~> ApiKeyHeader ~> route ~> check {
      ...
      "at existing height" in {
        ...
        Get(routePath(s"/stateHash/$lastButOneHeight")) ~> ApiKeyHeader ~> route ~> check {
          status shouldBe StatusCodes.OK
          responseAs[JsObject] shouldBe expectedResponse
        }
      }
  ...
  routePath("/validate") - {
    ...
    "NoSuchElementException" in {
      validatePost(TxHelpers.invoke()) ~> ApiKeyHeader ~> route ~> check {
      ...
      testFunction(
        ...
        tx =>
          s"""[ {
             ...
             |    } ],
             |    "transfers" : [ {
             |      "address" : "${TxHelpers.secondAddress}",
             |      "asset" : "${issue.asset}",
             |      "amount" : 1
             |    } ],
             |    "issues" : [ ],
    ...
    "invoke tx returning leases" in {
      ...
      val amount1    = 100
      val recipient1 = Recipient.Address(ByteStr.decodeBase58("3DdAmAhx8nwm8c6rEYnabSMJkayZGv4TUab").get)
      val nonce1     = 0
      ...
      domain.appendBlock(
        ...
        TxHelpers.setScript(
          ...
          TestCompiler(V6).compileContract(s"""@Callable(i)
                                              ...
                                              |  Lease(Address(base58'3DdAmAhx8nwm8c6rEYnabSMJkayZGv4TUab'), 100, 0),
      ...
      Post(routePath("/validate"), HttpEntity(ContentTypes.`application/json`, invoke.json().toString())) ~> ApiKeyHeader ~> route ~> check {
        val json = responseAs[JsValue]
        (json \ "valid").as[Boolean] shouldBe true
    ...
    "InvokeExpression" in {
      ...
      jsonPost(routePath("/validate"), invokeExpression.json()) ~> ApiKeyHeader ~> route ~> check {
        val json = responseAs[JsValue]
