...
import com.decentralchain.api.http.RewardApiRoute
...
class RewardApiRouteSpec extends RouteSpec("/blockchain") with WithDomain {
  ...
  val settingsWithoutAddresses: DCCSettings = RideV6.copy(blockchainSettings =
  ...
  private def checkWithSettings(settings: DCCSettings, height: Option[Int] = None) =
