...
import com.decentralchain.consensus.{FairPoSCalculator, PoSCalculator}
import com.decentralchain.features.BlockchainFeatures
import com.decentralchain.features.BlockchainFeatures.LightNode
import com.decentralchain.features.MultiPaymentPolicyProvider.*
...
import com.decentralchain.lang.v1.compiler.Terms.{EVALUATED, FUNCTION_CALL}
import com.decentralchain.lang.v1.evaluator.{Log, ScriptResult}
...
import com.decentralchain.state.BlockRewardCalculator.CurrentBlockRewardPart
import com.decentralchain.state.diffs.invoke.InvokeScriptDiff.validateIntermediateBalances
import com.decentralchain.state.diffs.invoke.{InvokeScript, InvokeScriptDiff, InvokeScriptTransactionLike}
import com.decentralchain.transaction.Asset.*
import com.decentralchain.transaction.TxValidationError.{FailedTransactionError, GenericError}
...
import com.decentralchain.transaction.smart.script.trace.CoevalR.traced
import com.decentralchain.transaction.smart.script.trace.InvokeScriptTrace
import com.decentralchain.transaction.transfer.TransferTransaction
import com.decentralchain.transaction.{Asset, DiffToLogConverter, TransactionBase, TransactionType}
import monix.eval.Coeval

import scala.util.Try

object DCCEnvironment {
  ...
  def apply(
      ...
      ds: DirectiveSet,
      txId: ByteStr
  ): DCCEnvironment = new DCCEnvironment(nByte, in, h, blockchain, tthis, ds, txId, blockchain)
}

class DCCEnvironment(
    nByte: Byte,
    in: Coeval[Environment.InputEntity],
    h: Coeval[Int],
    blockchain: Blockchain,
  ...
  def toAddress(recipient: Recipient): Option[com.decentralchain.account.Address] = {
    recipient match {
      case Address(bytes) =>
        com.decentralchain.account.Address
          .fromBytes(bytes.arr)
          .toOption
      case Alias(name) =>
        com.decentralchain.account.Alias
          .create(name)
  ...
  override def resolveAlias(name: String): Either[String, Recipient.Address] =
    ...
    blockchain
      .resolveAlias(com.decentralchain.account.Alias.create(name).explicitGet())
  ...
  override def accountBalanceOf(addressOrAlias: Recipient, maybeAssetId: Option[Array[Byte]]): Either[String, Long] = {
    (for {
      aoa <- addressOrAlias match {
        ...
        case Alias(name)    => com.decentralchain.account.Alias.create(name)
      }
...
object DAppEnvironment {
  ...
  final case class DAppInvocation(
      dAppAddress: com.decentralchain.account.Address,
      call: FUNCTION_CALL,
...
class DAppEnvironment(
    ...
    currentDApp: com.decentralchain.account.Address,
    currentDAppPk: com.decentralchain.account.PublicKey,
    calledAddresses: Set[com.decentralchain.account.Address],
...
) extends DCCEnvironment(nByte, in, h, blockchain, tthis, ds, tx.id(), blockchain)
