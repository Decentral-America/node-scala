...
import com.decentralchain.transaction.validation.{TxValidator, ValidatedNV, ValidatedV}
...
object InvokeScriptTxValidator extends TxValidator[InvokeScriptTransaction] {
  def checkAmounts(payments: Seq[Payment]): ValidatedNV = {
    ...
    if (invalid.nonEmpty)
      Invalid(NonEmptyList.fromListUnsafe(invalid.toList).map(p => NonPositiveAmount(p.amount, p.assetId.fold("Dcc")(_.toString))))
    else Valid(())
  }
