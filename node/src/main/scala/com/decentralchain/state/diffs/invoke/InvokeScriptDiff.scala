...
import com.decentralchain.lang.v1.evaluator.{ContractEvaluator, IncompleteResult, Log, ScriptResult, ScriptResultV3, ScriptResultV4}
import com.decentralchain.lang.v1.traits.Environment
import com.decentralchain.lang.v1.traits.domain.Tx.{InvokePseudoTx, ScriptTransfer}
...
import com.decentralchain.transaction.smart.script.trace.{AssetVerifierTrace, CoevalR, TracedResult}
...
import com.decentralchain.transaction.{TransactionType, TxValidationError}
  ...
  private def ensurePaymentsAreNotNegative(blockchain: Blockchain, tx: InvokeScript, invoker: Address, dAppAddress: Address) = traced {
    tx.payments.collectFirst {
      case p if p.amount < 0 =>
        s"DApp $invoker invoked DApp $dAppAddress with attached ${p.assetId.fold("DCC")(a => s"token $a")} amount = ${p.amount}"
    } match {
