...
import com.decentralchain.state.StateHashBuilder.Result
...
class StateHashBuilder {
  ...
  def addDccBalance(address: Address, balance: Long): Unit = {
    addEntry(SectionId.DccBalance, address.bytes)(Longs.toByteArray(balance))
  }
