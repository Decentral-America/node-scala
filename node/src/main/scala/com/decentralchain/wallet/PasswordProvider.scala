package com.decentralchain.wallet
  ...
  def askPassword(): String = {
    ...
    Option(System.console()) match {
      case None =>
        ...
        log.error(
          "Probably, it happens because you trying to start Dcc node using supervisor service (like systemd) without specified wallet password."
        )
