...
class MakeStringTest extends EvaluatorSpec {
  ...
  property("makeString input limit") {
    Seq(V4, V5).foreach { version =>
      ...
      eval(script)(using version, checkNext = false) should produce(s"List size should not exceed $MaxListLengthV4")
    ...
    eval(script)(using V6) should produce("Input list size = 71 for makeString should not exceed 70")
  ...
  property("makeString output limit") {
    Seq(V4, V5).foreach { version =>
      ...
      eval(script2)(using version, checkNext = false).explicitGet().asInstanceOf[CONST_STRING].s.length shouldBe 32767
    }

    val script = s""" [${s""" "${"a" * 10}", """ * 25} "${"a" * 201}"].${PureContext.makeString_V6.name}(", ") """
    eval(script)(using V6) should produce("Constructing string size = 501 bytes will exceed 500")
    // 10 * 25 + 201 + 2 * 25 = 501

    val script2 = s""" [${s""" "${"a" * 10}", """ * 25} "${"a" * 200}"].${PureContext.makeString_V6.name}(", ") """
    eval(script2)(using V6).explicitGet().asInstanceOf[CONST_STRING].s.length shouldBe 500
  }
  ...
  property("makeString function family output limit") {
    ...
    eval(script)(using V6) should produce("Constructing string size = 6001 bytes will exceed 6000")
