...
class EvaluatorV1V2Test extends PropSpec with EitherValues {
  ...
  private def eval[T <: EVALUATED](
      ...
      expr: EXPR
  )(using version: StdLibVersion): Either[ExecutionError, T] = {
  ...
  private def evalPure[T <: EVALUATED](context: EvaluationContext[NoContext, Id] = pureEvalContext, expr: EXPR)(using
      version: StdLibVersion
  ): Either[ExecutionError, T] =
  ...
  private def recArrWeight(script: String): Either[ExecutionError, EVALUATED] = {
    ...
    com.decentralchain.lang.v1.parser.Parser.parseExpr(script) match {
