# Testnet Final Source — Waves-Identical Feature Registry, DCC Improvements Unconditional (rev. 5)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
>
> **Rev. 5 (2026-09-02):** task order changed on request — the two debugging tasks (finding + fixing the state-hash non-determinism at height 2640) moved to the END, immediately before the relaunch task they gate, instead of sitting in the middle of the mechanical work. Renumbered: what was infra-workflows/schema/scalafmt/docs/final-review (old Task 4/5/6/7/8) are now Task 3/4/5/6/7; the message-fix + real debugging (old Task 3/3b) are now Task 8/8b; the relaunch (old Task 9) stays Task 9. Every cross-reference in the plan body was updated to match — verified by grep, not assumed. No content changed from rev. 4 beyond the reorder and its cross-references; all 61 rev.4 findings remain in force.
>
> **Rev. 4** (content, superseded by rev. 5's reorder only — findings still apply): a full re-audit against the real post-merge `dev` tree (61 findings) folded in. Task 1's file list was materially incomplete (10 missing test files, 2 missing config files) and contained instructions that would have caused an executor to delete working code or misjudge a full rewrite as a touch-up; Task 2's "votable, not pre-activated" decision had no path to ever actually activate (dead on arrival) — paired with a companion activation step; the debugging task's premise about what the frozen chain was running was **factually wrong** — corrected, with a new Step 0 that may resolve it as a one-command `git bisect`; the schema task had an unflagged pom-version mismatch that could make its "successful" publish silently republish the wrong version; the "tasks run in parallel" claim was wrong for two task pairs.
>
...
- **[rev.4] Task ordering is NOT fully parallel.** Parallel-safe: Task 8b, Task 3, Task 4 Step 1. Strictly sequential: **Task 1 → Task 5 → Task 2 → Task 6 → Task 7** (Task 5's `scalafmtAll` must run after Task 1's ~50-file rewrite, or its "whitespace-only" check is meaningless and it will conflict with Task 1's own renames; Task 6 must run after Task 2, since it verifies Task 2's divergence-doc section and reconciles the genesis config's feature list against Task 2's decision). Task 4 Step 2 gates on Task 4 Step 1 succeeding, verified (see Task 4). Task 9 gates on Task 8b AND Task 7.
...
- Modify: `block/BlockEndorsement.scala` (:27 `_ENDORSE_` unconditional), `consensus/hotstuff/HotStuffQuorum.scala` (:64 `_HSVOTE_` unconditional; drop `cryptoV2` from `verifyVote`/`formQC`/`verifyQC`), `HotStuffEquivocationProof.scala` (dst = HSVOTE constant), `HotStuffCoordinator.scala`, `HotStuffEngine.scala` (`EngineState.cryptoV2`), `HotStuffVotePool.scala`, `NodeHotStuffEffects.scala` (`signVote(dst)` → constant), `transaction/CommitToGenerationTransaction.scala` (:70-77 v2 branch only; drop `cryptoV2` from `popMessage`/`popDst`/`mkPopSignature`), `transaction/TransactionFactory.scala`, `state/BlockEndorser.scala` (remove `carrierHeight` :110/:123/:154 and the era read at :160 — its ONLY use), `state/EndorsementFilter.scala` (drop `cryptoV2` field), `state/EndorsementStorage.scala`, `state/diffs/BlockDiffer.scala` (:643), `state/diffs/CommitToGenerationTransactionDiff.scala` (:26), `settings/BlockchainSettings.scala` (:155 MAINNET `BlsCryptoV2.id -> 1`, :181 STAGENET — delete), `Application.scala` (:351-355 provider), `api/http/TransactionsApiRoute.scala` (:261), `api/http/requests/CommitToGenerationRequest.scala`, **`utils/UtilApp.scala` — see the dedicated note below, DO NOT follow a naive "delete tags at :415-423" instruction**, `utx/UtxPoolImpl.scala` (comment), `node/src/main/resources/network-defaults.conf:47` (`30 = 1` → delete), `application.conf:20-24` (the BlsCryptoV2 comment block), **`docker/private/decentralchain.custom.conf`** (`[rev.4] MISSING FROM PRIOR REVS — added by the same commit that pre-activated feature 30; delete its `30 = 1`/equivalent pre-activation line`), `node/testkit/.../TxHelpers.scala` (`cryptoV2` param on `commitToGeneration`)
  ...
  **[rev.4] `Miner.scala:211-216` — do not touch these in Task 1; they're Task 8's job.** `:211-213` is the generating-balance-floor veto message (`"$address is not committed on $newBlockHeight..."`); `:214-216` is a SEPARATE, structurally identical conflict-veto message (`"$address is conflict on $newBlockHeight..."`) — leave both alone here, Task 8 fixes only the first one's wording.
...
### Task 3: Make the commit-generator workflows fail loudly and verify the committee
...
### Task 4: Publish protobuf-schemas 1.6.6 to Maven Central (operator) and remove ONLY the schema source-build step
...
### Task 5: scalafmt debt to zero in TEST sources and gate them
...
### Task 6: Docs closure — no document may still describe the gated design
...
### Task 7: Final whole-branch review, full-suite gate, image build, merge, tag
...
### Task 8: ✅ DONE — root-cause the 2026-09-01 stall from node logs; fix the misleading miner message
...
**⛔ PRECONDITION: Task 8b must be complete, fixed, and verified before this task runs.** Do not write or execute a relaunch runbook while Task 8b is open.
...
2. ✅ **Node logs — RESOLVED:** root cause found (Task 8). **[rev.4] The premise about what code was running was corrected — see Task 8b's rewrite.**
3. ⛔ **Task 8b is the critical-path blocker.** Task 9 must not run until it's solved.
4. ⛔ **Maven Central publish** of protobuf-schemas 1.6.6 + environment-gate approval — blocks Task 4 Step 2. **[rev.4] Pre-check the pom-version rewrite mechanism first (see Task 4 Step 1) or the publish may silently be a no-op for the version that matters.**
5. Optional `NVD_API_KEY` secret.
6. ✅ **Testnet disposability — CONFIRMED.**
7. ⛔ Live go-aheads still needed: Task 3 Step 4 workflow dispatch; all of Task 9.
8. **The SSH key used for Task 8's log pull must be rotated** — it was pasted into a chat session.

## Self-Review

- **[rev.4b, reorder]** Debugging tasks moved to the end, right before what they gate: Task 3 (infra automation), Task 4 (schema), Task 5 (scalafmt), Task 6 (docs), Task 7 (final review/merge/tag) all come first — none of them depend on the debugging outcome. Task 8 (misleading-message fix) and Task 8b (the real state-hash debugging) now sit last, immediately before Task 9 (relaunch), which is the only task that actually gates on them. This lets everything mechanical run and land while the debugging proceeds in parallel or after, without blocking on it.
- Spec coverage: registry parity (T1, T2 + activation path), unconditional improvements (T1), automation (T3), schema (T4 — pom pre-check added), format gate (T5 — reordered after T1), docs + version (T6 — reordered after T2), final gate + image + tag (T7 — Release-vs-tag clarified), stall root cause (T8 — done) + actual fix (T8b — corrected premise, new Step 0 bisect), relaunch + soak (T9 — feature-26 activation config added, gated on T8b + T7). All 61 audit findings folded in (P0/P1/P2 from the audit report).
- Placeholder scan: ⛔ steps are operator gates or genuine unsolved debugging, and say so.
- Type consistency: `popMessage(chainId, sender, endorserPk, periodStart)`, `PopDst`, `VoteDst`, the renamed `Bls*DomainSeparationTag` constants named identically across T1/T6; `/generators/at/{height}` semantics identical in T3 and T9.
- Ordering fixed: **T1 → T5 → T2 → T6 → T7** strictly (mechanical work); **T8 → T8b → T9** strictly (debugging then relaunch); T3/T4-step-1 may run in parallel with anything; T9 additionally gates on T7.
