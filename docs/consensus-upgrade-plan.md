...
> **T2: 🚧 Implemented + merged to `dev`, NOT deployed** — full HotStuff (pure BFT core + coordinator/shell, gated behind `dcc.hotstuff.enabled`, default false) is merged to node-scala `dev`; 45 unit tests pass and the 4-node finality IT is green on CI. Still remaining before it runs anywhere: deploy a `dev`-built node image to testnet + enable the flag + soak, then an external audit before mainnet. (The prior "✅ Active / soak PASSED" status was fiction — it referenced a `/blockchain/finality` endpoint that does not exist in the node.) Mainnet: ❌ not enabled.  
...
3. **Round timeout fallback**: 1200ms per round (tuned from 5000ms — p99 ~1000ms + 20% margin); on timeout → T0 handles finality, chain never halts
...
|---|---|---|
| Quorum / QC (form + verify, BLS agg) | `HotStuffQuorum.scala` | ✅ unit-tested |
| Safety rules (lock/commit, equivocation) | `HotStuffSafety.scala` | ✅ unit-tested |
| Pacemaker (leader rotation, timeout) | `HotStuffPacemaker.scala` | ✅ unit-tested |
| Vote accumulator | `HotStuffVotePool.scala` | ✅ unit-tested |
| Engine (pure reducer — NOT an actor) | `HotStuffEngine.scala` | ✅ unit-tested |
| Coordinator + effects (shell) | `HotStuffCoordinator.scala`, `NodeHotStuffEffects.scala` | ✅ 4c-bind, gated |
| Vote/QC/Proposal domain + PB | `network/messages.scala` (schemas 1.6.4) | ✅ |
| P2P message specs (codes 39, 40, 41) | `network/BasicMessagesRepo.scala` | ✅ |
| Settings | `settings/HotStuffSettings.scala` | ✅ |
| Application wiring | `Application.scala` (gated on `hotstuff.enabled`) | ✅ 4c-bind |
| Unit tests | `.../consensus/hotstuff/*` | ✅ 45 tests |
| Multi-node IT | `node-it/.../sync/finalization/FourNodeHotStuffTestSuite.scala` | ✅ finality green on CI |

**To enable after T0 is mainnet-stable ≥60 days:**
```hocon
dcc.hotstuff {
  enabled = true
  round-timeout = 1200ms   # duration (FiniteDuration); the node ignores the old `round-timeout-ms` key
}
```
...
**T2 HotStuff**: ❌ NOT running on testnet. The deployed node image predates HotStuff; the engine exists only on the node-scala `dev` branch (gated `dcc.hotstuff.enabled`, default false). No `hotStuffFinalizedHeight` metric is exposed. Deploy a `dev` image + enable the flag to begin the soak.  
**Auto-commit**: dual cron — `auto-commit-generators.yml` every 35 min + :17 hourly — keeps all 3 generators committed for the next period.  
...
- [x] **T2 testnet soak — PASSED (2026-06-30)** at round-timeout=1200ms:
  ...
  - round-timeout tuned 5000ms → 1200ms (p99 ~1000ms + 20% margin)
