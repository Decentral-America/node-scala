...
import com.decentralchain.account.{Address, PublicKey, SeedKeyPair}
...
import com.decentralchain.events.fixtures.InvokeDccTxCheckers.checkSimpleInvoke
...
import com.decentralchain.transaction.TxHelpers.secondSigner
import com.decentralchain.transaction.assets.*
import com.decentralchain.transaction.assets.exchange.*
import com.decentralchain.transaction.assets.exchange.Order.Version
...
import com.decentralchain.transaction.utils.EthConverters.EthereumKeyPairExt
import com.decentralchain.transaction.{Asset, CreateAliasTransaction, DataTransaction, EthereumTransaction, Transaction, TxHelpers}
...
class BlockchainUpdatesTestBase extends FreeSpec with WithBUDomain with ScalaFutures {
  ...
  val firstTxParticipantEthereumAddress: Address       = ethKeyPairExt.toEthDccAddress
  ...
  val firstTxParticipantBalanceBefore: Long            = 20.dcc
  ...
  val secondTxParticipantBalanceBefore: Long           = 20.dcc
  ...
  protected def checkAlias(append: Append, aliasTx: CreateAliasTransaction)(implicit pos: Position): Unit = {
    ...
    checkBalances(
      ...
      Map((firstTxParticipantAddress, Dcc) -> (firstTxParticipantBalanceBefore, firstTxParticipantBalanceAfter))
    )
  }
  ...
  protected def checkTransferTx(append: Append, transferTx: TransferTransaction)(implicit pos: Position): Unit = {
    ...
    checkBalances(
      ...
      Map(
        ...
        (secondTxParticipantAddress, Dcc) -> (secondTxParticipantBalanceBefore, secondTxParticipantBalanceAfter)
      )
    )
  }
  ...
  protected def checkLeaseTx(append: Append, lease: LeaseTransaction)(implicit pos: Position): Unit = {
    ...
    checkBalances(
      filterOutMinerBalanceUpdates(append),
      Map(
        (firstTxParticipantAddress, Dcc) -> (firstTxParticipantBalanceBefore, firstTxParticipantBalanceBefore - customFee)
      )
    )
  ...
  protected def checkDataTransfer(append: Append, data: DataTransaction)(implicit pos: Position): Unit = {
    ...
    checkBalances(
      filterOutMinerBalanceUpdates(append),
      Map((firstTxParticipantAddress, Dcc) -> (firstTxParticipantBalanceBefore, firstTxParticipantBalanceBefore - customFee))
    )
  ...
  protected def checkSetAssetScript(append: Append, setAssetScript: SetAssetScriptTransaction, issue: IssueTransaction)(implicit
    ...
    checkBalances(
      filterOutMinerBalanceUpdates(append),
      Map(
        (firstTxParticipantAddress, Dcc) -> (firstTxParticipantBalanceBeforeSetAssetScriptTx, firstTxParticipantBalanceAfterSetAssetScriptTx)
      )
    )
  ...
  protected def checkUpdateAssetInfo(append: Append, updateAssetInfo: UpdateAssetInfoTransaction)(implicit pos: Position): Unit = {
    ...
    checkBalances(
      filterOutMinerBalanceUpdates(append),
      Map(
        (firstTxParticipantAddress, Dcc) -> (firstTxParticipantBalanceBeforeUpdateAssetInfoTx, firstTxParticipantBalanceAfterUpdateAssetInfoTx)
      )
    )
  ...
  protected def checkSponsorFee(append: Append, sponsorFee: SponsorFeeTransaction, balanceBefore: Long, balanceAfter: Long)(implicit
    ...
    checkBalances(
      filterOutMinerBalanceUpdates(append),
      Map((firstTxParticipantAddress, Dcc) -> (balanceBefore, balanceAfter))
    )
    checkAssetsStateUpdates(assetDetails.before, firstToken, isNft = false, firstTokenQuantity)
