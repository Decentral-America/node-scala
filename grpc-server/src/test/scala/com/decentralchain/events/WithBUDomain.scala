...
trait WithBUDomain extends WithDomain { suite: Suite =>
  ...
  def withDomainAndRepo(settings: DCCSettings)(f: (Domain, Repo) => Unit, wrapDB: RocksDB => RocksDB = identity): Unit = {
  ...
  def withManualHandle(settings: DCCSettings, setSendUpdate: (() => Unit) => Unit)(f: (Domain, Repo) => Unit): Unit =
    withDomain(settings) { d =>
      tempDb { rdb =>
  ...
  def withNEmptyBlocksSubscription(count: Int = 2, request: SubscribeRequest = SubscribeRequest.of(1, Int.MaxValue), settings: DCCSettings)(
