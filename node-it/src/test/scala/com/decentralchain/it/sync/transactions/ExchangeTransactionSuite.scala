...
import com.decentralchain.api.http.ApiError.{CustomValidationError, StateCheckFailed}
import com.decentralchain.common.utils.EitherExt2.*
import com.decentralchain.features.BlockchainFeatures
import com.decentralchain.it.api.SyncHttpApi.*
import com.decentralchain.it.sync.*
import com.decentralchain.it.sync.smartcontract.exchangeTx
...
class ExchangeTransactionSuite extends BaseTransactionSuite with NTPTime {
  ...
  test("negative - check orders v2 and v3 with exchange tx v1") {
    ...
    val pair = AssetPair.createAssetPair("DCC", exchAsset.id().toString).get
  ...
  test("exchange tx with orders v3") {
    ...
    for (
      ...
      val assetPair  = AssetPair.createAssetPair("DCC", assetId.toString).get
      val buy = Order
