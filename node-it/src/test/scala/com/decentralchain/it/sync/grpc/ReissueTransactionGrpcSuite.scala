...
class ReissueTransactionGrpcSuite extends GrpcBaseTransactionSuite with NTPTime {
  ...
  test("asset reissue changes issuer's asset balance; issuer's dcc balance is decreased by fee") {
    for (v <- reissueTxSupportedVersions) {
      val reissuerBalance    = sender.dccBalance(reissuerAddress).available
      val reissuerEffBalance = sender.dccBalance(reissuerAddress).effective
      ...
      sender.dccBalance(reissuerAddress).available shouldBe reissuerBalance - issueFee - reissueFee
      sender.dccBalance(reissuerAddress).effective shouldBe reissuerEffBalance - issueFee - reissueFee
  ...
  test("can't reissue not reissuable asset") {
    for (v <- reissueTxSupportedVersions) {
      ...
      sender.dccBalance(reissuerAddress).available shouldBe reissuerBalance - issueFee
      sender.dccBalance(reissuerAddress).effective shouldBe reissuerEffBalance - issueFee
  ...
  test("not able to reissue if cannot pay fee - insufficient funds") {
    for (v <- reissueTxSupportedVersions) {
      val reissuerBalance    = sender.dccBalance(reissuerAddress).available
      val reissuerEffBalance = sender.dccBalance(reissuerAddress).effective
      val hugeReissueFee     = reissuerEffBalance + 1.dcc
  ...
  test("able to transfer new reissued amount of assets") {
    for (v <- reissueTxSupportedVersions) {
      ...
      sender.dccBalance(reissuerAddress).available shouldBe reissuerBalance - issueFee - reissueFee - minFee
      sender.dccBalance(reissuerAddress).effective shouldBe reissuerEffBalance - issueFee - reissueFee - minFee
