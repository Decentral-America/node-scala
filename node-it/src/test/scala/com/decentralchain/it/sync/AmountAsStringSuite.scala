...
import com.decentralchain.it.api.{Transaction, TransactionInfo}
import com.decentralchain.state.{Height, IntegerDataEntry}
...
import com.decentralchain.transaction.{CreateAliasTransaction, TxExchangeAmount, TxExchangePrice, TxVersion}
...
class AmountAsStringSuite extends BaseFunSuite with ScorexLogging {
  override protected def nodeConfigs: Seq[Config] = Seq(
    Miners(1).quorum(0).overrides("dcc.miner.micro-block-interval = 5s"), // when UTX is empty, retry building microblock in 2 seconds
    NotMiner
  )
  ...
  test("amount as string in exchange transaction") {
    ...
    val buyOrder = Order
      .buy(
        ...
        exchanger,
        exchanger.publicKey,
        AssetPair.createAssetPair("DCC", exchAssetId).get,
        amount,
        price,
        ts,
