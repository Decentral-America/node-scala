...
import com.decentralchain.api.http.ApiError.ScriptCompilerError
...
class RideUpdateAssetInfoTxSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("can check UpdateAssetInfo tx from contracts") {
    val asset2Height = Height(sender.transactionInfo[TransactionInfo](asset2.id.toString).height)
