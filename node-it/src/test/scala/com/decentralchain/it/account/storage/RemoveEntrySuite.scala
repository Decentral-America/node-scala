...
import com.decentralchain.lang.v1.compiler.Terms.{CONST_BOOLEAN, CONST_BYTESTR, CONST_LONG, CONST_STRING}
...
class RemoveEntrySuite extends BaseFreeSpec {
  ...
  "Remove entry from account storage" - {
    for (data <- Seq(stringTestData, integerTestData, booleanTestData, binaryTestData)) s"${data.ct}Entry could be removed from account storage" in {
      ...
      miner.getDataByKey(address, data.k).getClass.getCanonicalName shouldBe s"com.decentralchain.state.${data.ct}DataEntry"
