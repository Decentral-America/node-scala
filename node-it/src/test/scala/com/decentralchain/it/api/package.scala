...
import com.decentralchain.utils.{Paged, ScorexLogging}
...
package object api {
  ...
  implicit val addressReads: Reads[com.decentralchain.account.Address] = Reads {
  ...
  implicit val dstMapReads: Reads[Map[com.decentralchain.account.Address, Long]] = Reads { json =>
    json.validate[Map[String, Long]].map { dst =>
      dst.map { case (addrStr, balance) =>
        com.decentralchain.account.Address.fromString(addrStr).explicitGet() -> balance
      }
    }
  }
