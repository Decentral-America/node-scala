...
import com.decentralchain.account.{AddressOrAlias, KeyPair, SeedKeyPair}
import com.decentralchain.api.http.RewardApiRoute.RewardStatus
import com.decentralchain.api.http.requests.{CommitToGenerationRequest, IssueRequest}
import com.decentralchain.api.http.{ApiError, DebugMessage}
import com.decentralchain.common.state.ByteStr
import com.decentralchain.common.utils.EitherExt2.*
import com.decentralchain.features.BlockchainFeatures
import com.decentralchain.features.api.{ActivationStatus, FeatureActivationStatus}
...
import com.decentralchain.state.{AssetDistribution, AssetDistributionPage, DataEntry, GenerationPeriod, Height}
...
import com.decentralchain.transaction.{Asset, TransactionType, TxExchangeAmount, TxExchangePrice, TxVersion}
...
object SyncHttpApi extends Assertions with matchers.should.Matchers {
  ...
  implicit class NodeExtSync(n: Node) extends Assertions with matchers.should.Matchers {
    import com.decentralchain.it.api.AsyncHttpApi.NodeAsyncHttpApi as async
  ...
  implicit class NodesExtSync(nodes: Seq[Node]) {
    ...
    import com.decentralchain.it.api.AsyncHttpApi.NodesAsyncHttpApi as async
    ...
    def rollbackWithoutBlacklisting(height: Height, returnToUTX: Boolean = true): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.decentralchain.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).rollback(height, returnToUTX)
        },
        ConditionAwaitTime
      )
    }
    ...
    def waitForHeight(height: Height): Unit = {
      sync(
        Future.traverse(nodes) { node =>
          com.decentralchain.it.api.AsyncHttpApi.NodeAsyncHttpApi(node).waitForHeight(height)
        },
        ConditionAwaitTime
      )
    }
